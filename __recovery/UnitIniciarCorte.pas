...
begin
    ...
    begin
        Application.MessageBox('Selecione o produto', 'Ordem de Corte',MB_OK + MB_ICONWARNING);
        exit;
    end;
    ...
    begin
        ...
        'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end;
    if dmOrdemCorte.qyOrdemIniciarCorte.FieldByName('oc_situacao').Value = 3 then
    begin
        Application.MessageBox('Ordem de corte finalizada, por esse motivo é vedado qualquer modificação na ordem de corte', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end;
...
begin
    ...
    begin
        ...
        ExecSQL;
        tratarDataHora;
        Application.MessageBox('Ordem alterada com sucesso!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
...
begin
    ...
    begin
        Application.MessageBox('Não é permitido editar corte já finalizado!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end
    else
    ...
    begin
        if MessageBox('Deseja cancelar esta ordem de corte?', 'Ordem Corte', MB_ICONQUESTION + MB_YESNO + MB_APPLMODAL) = IDYES then
            ...
            begin
                ...
                ExecSQL;
                Application.MessageBox('Ordem cancelada com sucesso!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
                butSairInicioCorteClick(Sender);
...
begin
    ...
    begin
        ...
        'Ordem de Corte', MB_OK + MB_ICONWARNING);
        Application.MessageBox('Solicite ao almoxarifado que processe o recebimento da devolução dos empenhos!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end;
    ...
    begin
        Application.MessageBox(PChar('Grade incorreta na Ficha Tecnica: '+ intTostr(dmOrdemCorte.qyFichaId.FieldByName('fi_id').Value)), 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end;
    ...
    begin
        Application.MessageBox('Ficha tecnica sem grade informada!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        exit;
    end;
    ...
    begin
        ...
        begin
            Application.MessageBox('Ordem de corte não pode ser criada porque existe linha 120 lançada na costura!','Ordem de Corte', MB_OK + MB_ICONWARNING);
            Application.MessageBox('Linha Título 120 não é comum ser usado na fase de costura quando o segmento do tecido for JEANS ou SARJA.','Ordem de Corte', MB_OK + MB_ICONWARNING);
            with application do
            begin
    ...
    begin
        ...
        begin
            Application.MessageBox('Existe artigo na ficha tecnica sem fase informada!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
            exit;
        end;
    end;
    ...
    begin
        ...
        begin
            Application.MessageBox('Há mais de um tecido principal na ficha tecnica, por favor realize o ajuste!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
            exit;
        end;
    end;
    ...
    begin
        ...
        begin
            Application.MessageBox('Cor de referência diferente da cor do tecido principal!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
            exit;
        end;
     end;
    ...
    begin
        ...
        begin
            ...
            begin
                Application.MessageBox('Já existe uma ordem de corte protótipo para essa referencia!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
                exit;
            end
            else
                with application do
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                    Application.MessageBox('Prototipo em espera por aprovação!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
                    exit;
                end;
                if dmOrdemCorte.qyGEComOuSemProt.FieldByName('fp_situacao').Value = 'R' then
                begin
                    Application.MessageBox('Protótipo reprovado, por esse motivo produção não pode ser realizada', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
                    exit;
                end;
            end
            else
    ...
    begin
         ...
         begin
              ...
              begin
                  ...
                  begin
                      Application.MessageBox(PChar(msg), 'Ordem de Corte',MB_OK + MB_ICONWARNING);
                      exit;
                  end;
              end;
...
begin
    ...
    butNovo.Enabled := true;
    butEscolherCores.Enabled := true;
    Application.MessageBox('Operação realizada com sucesso!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
end;
...
begin
    ...
    begin
        Application.MessageBox('Data de finalização do corte não pode ser anterior a atual!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if horaOrdemFinalizacao.Time < horaSolicitacao.Time then
    begin
        Application.MessageBox('Horário de finalização do corte não pode ser anterior a atual!','Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if dataCortePrevisto.Date < dataSolicitacao.Date then
    begin
         Application.MessageBox('Data de inicio do corte previsto não pode ser anterior a data de solicitação do corte!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
         abort;
    end;

    if horaCortePrevisto.Time < horaSolicitacao.Time then
    begin
        Application.MessageBox('Horário de inicio do corte previsto não pode ser anterior ao horário de solicitação do corte!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if dataFinalCortePrevisto.Date < dataCortePrevisto.Date then
    begin
        Application.MessageBox('Data de finalização do corte previsto não pode ser anterior a data de inicio do corte previsto!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if horaFinalCortePrevisto.Time < horaCortePrevisto.Time then
    begin
        Application.MessageBox('Horário de finalização do corte previsto não pode ser anterior ao horário de inicio do corte previsto!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if dataRealCortado.Date < dataFinalCortePrevisto.Date then
    begin
        Application.MessageBox('Data de inicio do real cortado não pode ser anterior a data de finalização do corte previsto!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if horaRealCortado.Time < horaFinalCortePrevisto.Time then
    begin
        Application.MessageBox('Horário de inicio do real cortado não pode ser anterior ao horário de finalização do corte previsto!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if dataFinalRealCortado.Date < dataRealCortado.Date then
    begin
        Application.MessageBox('Data de finalização do real cortado não pode ser anterior a data de inicio do real cortado!', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;

    if horaFinalRealCortado.Time < horaRealCortado.Time then
    begin
        Application.MessageBox('Horário de finalização do corte real não pode ser anterior ao horário de inicio do corte real', 'Ordem de Corte', MB_OK + MB_ICONWARNING);
        abort;
    end;
end;
...
begin
    ...
    begin
        ...
        begin
            Application.MessageBox('Você não tem permissão para executar este procedimento','Atenção',mb_ok + mb_iconexclamation);
            abort;
        end
        else
            if dmOrdemCorte.qyCortePorTipoFichaId.RecordCount > 0 then
